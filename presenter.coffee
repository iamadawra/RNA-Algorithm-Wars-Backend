@PageData = {
  ...
  get_me : (filter, success_cb, fail_cb) ->
    
    if !success_cb
      return
    
    url = Application.GET_URI
    AjaxManager.query("GET", url, {type:"me", filter:filter, random:Math.random()}, (response) =>
      EternaUtils.process_response(response, (data) =>
        success_cb(data)
      )
    , fail_cb)

  get_newsfeed : (pageindex, filter, success_cb, fail_cb) ->
    
    if !success_cb
      return
    url = Application.GET_URI
    AjaxManager.query("GET", url, {type:"newsfeed", skip:pageindex * 4, size:4, filter:filter}, (response) =>
      EternaUtils.process_response(response, (data) =>
        success_cb(data)
      )
    , fail_cb)
  ...
  get_group : (nid, success_cb, fail_cb) ->
    if !success_cb
      return
      
    query_params = {type:"group", nid:nid}
  ...
  get_player : (uid, tab_type, success_cb, fail_cb) ->
    if !success_cb
      return
      
    query_params = {type:"user", uid:uid, tab_type:tab_type}
  ...
  get_labs : (lab_type, params, success_cb, fail_cb) ->
    ...
    if !(lab_type?)
      lab_type = "active_labs"  
      
    query_params = {type:lab_type, single:params['single'], switch:params['switch'], search:params['search'], skip:params['skip'], size:params['size'], sort:params['sort']}
  ...
  get_solution_info : (solid, nid, round, success_cb, fail_cb) ->
    if !success_cb
      return
      
    query_params = {type:"solution_info", solid:solid, nid:nid, round:round}
  ...
  get_synthesized_results : (params, success_cb, fail_cb) ->
    ...
    query_params = {type:"lab_synthesized_results", nid:params['nid']}
...
@Script = {
  ...
  get_script : (script_id, success_cb) ->
    url = Application.GET_URI
  ...
  post_script : (title, source, type, input, test, description, success_cb) ->
    url = Application.POST_URI
    params = {}
    params['type'] = "script"
    params['need'] = "save"
